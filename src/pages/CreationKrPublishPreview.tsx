...
import {
	buildBoardKey,
	findBoardByKey,
	formatBoardOptionLabel,
	groupBoardsByMajor,
	resolveInitialBoardKey,
} from "../utils/publishBoardUtils";
...
const CreationKrPublishPreview: React.FC = () => {
	...
	const reviewId = id ? Number(id) : Number.NaN;
	...
	const [boards, setBoards] = useState<
		Awaited<ReturnType<typeof publishApi.getBoards>>["boards"]
	>([]);

	const groupedBoards = useMemo(() => groupBoardsByMajor(boards), [boards]);
	const selectedBoard = useMemo(
		() => (selectedKey ? findBoardByKey(boards, selectedKey) : undefined),
		[boards, selectedKey],
	);
	...
	useEffect(() => {
		...
		const loadBoards = async () => {
			try {
				...
				setSelectedKey(
					resolveInitialBoardKey(response.boards, {
						suggestedSitePath: response.suggestedSitePath,
						suggestedBoardId: response.suggestedBoardId,
					}),
				);
			} catch (e) {
	...
	const handlePublish = async () => {
		...
		try {
			...
			const err = e as {
				response?: { data?: { publishError?: string; message?: string } };
			};
			setError(
			...
			{!loadingPreview && preview && (
				<div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
					...
					>
						...
						>
							리뷰 상태: {preview.reviewStatus} · 문서 상태:{" "}
							{preview.documentStatus}
							{preview.publishStatus &&
								` · 게시 상태: ${preview.publishStatus}`}
						</p>
					...
					>
						...
						{!loadingBoards && boards.length === 0 && (
							<p style={{ fontSize: "13px", color: "#b91c1c" }}>
								등록된 creation.kr 게시판이 없습니다. application.yml
								board-mappings 또는 카테고리 creationKrSitePath 설정을
								확인해주세요.
							</p>
						)}
							...
							<>
										...
										([major, groupBoards]) => (
											<optgroup key={major} label={major}>
												{groupBoards.map((board) => (
													<option
														key={buildBoardKey(board)}
														value={buildBoardKey(board)}
													>
														{formatBoardOptionLabel(board, suggestedLabel)}
													</option>
												))}
											</optgroup>
										),
								...
								{selectedBoard ? (
									...
									>
										...
										>
											{formatBoardOptionLabel(selectedBoard, suggestedLabel)}
										</div>
										{selectedBoard.majorCategory && (
											<div
												style={{
													fontSize: "12px",
													color: colors.secondaryText,
											...
											>
												분류: {selectedBoard.majorCategory}
											</div>
										)}
									</div>
								) : (
						...
						{!preview.publishable && preview.publishStatus !== "SUCCESS" && (
							...
							>
								현재 상태에서는 게시할 수 없습니다. 승인·완전 번역 조건을
								확인해주세요.
							</p>
						)}
